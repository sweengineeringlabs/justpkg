...
use swe_justpkg_nix::{sri_to_hex, nix_base32_to_hex, NixFetchError};
...
fn test_sri_to_hex_rejects_missing_algorithm_prefix() {
    ...
    assert!(result.is_err(), "bare base64 without 'sha256-' prefix must be rejected");
...
fn test_nix_base32_to_hex_all_zeros_is_valid() {
    ...
    assert!(hex.chars().all(|c| c.is_ascii_hexdigit()), "result must be lowercase hex");
}
