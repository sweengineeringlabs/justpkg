...
fn test_get_bytes_real_url_returns_nonempty_body() {
    ...
    let bytes = client
        .get_bytes("https://cache.nixos.org/nix-cache-info")
        .unwrap();
    assert!(!bytes.is_empty(), "cache info should not be empty");
    let text = String::from_utf8(bytes).unwrap();
    assert!(
        text.contains("StoreDir"),
        "cache info must contain StoreDir field"
    );
}
...
fn test_get_stream_real_url_writes_bytes_and_returns_count() {
    ...
    let mut buf = Vec::new();
    let n = client
        .get_stream("https://cache.nixos.org/nix-cache-info", &mut buf)
        .unwrap();
